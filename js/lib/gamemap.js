...
const GameMap = {
    ...
    addPattern(cell, pattern) {
        ...
        for (var row = 0; row < pattern.TILES_HIGH; row++) {
            for (var col = 0; col < pattern.TILES_WIDE; col++) {
                ...
                var newCell     = (row * layer.TILES_WIDE) + col + cell;
                var tileIndex   = pattern.data[ (row * pattern.TILES_WIDE) + col ];
    ...
    repeatPattern(cellArray, pattern) {
        ...
        const reducedArray          = cellArray.reduce((a, b) => a.concat(b), []).sort((a, b) =>  a - b);
        const startIdx              = reducedArray[0];
        const endIdx                = reducedArray[(reducedArray.length - 1)];
    ...
    renderEntity(idx) {
        let layer = this.getLayer('entities');
        console.log('placing entity...', idx);
        
        layer.renderEntity(idx);
    },
